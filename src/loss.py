...
class YoloLoss(nn.modules.loss._Loss):
    ...
    def forward(self, output, target):
        #made changes here accordingly
        class_output, bb_output = output
        batch_size = class_output.data.size(0)
        height = class_output.data.size(2)
        width = class_output.data.size(3)

        # Get x,y,w,h,conf,cls
        class_output = class_output.view(batch_size, self.num_anchors, -1, height * width)
        bb_output = bb_output.view(batch_size,self.num_anchors,-1,height*width)
        coord = torch.zeros_like(bb_output[:, :, :4, :]) 
        coord[:, :, :2, :] = bb_output[:, :, :2, :].sigmoid()  
        coord[:, :, 2:4, :] = bb_output[:, :, 2:4, :]
        conf = bb_output[:, :, 4, :].sigmoid()
        cls = class_output[:, :, :, :].contiguous().view(batch_size * self.num_anchors, self.num_classes,
