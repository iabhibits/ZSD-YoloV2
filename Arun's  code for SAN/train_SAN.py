...
from src.voc_dataset import VOCZSD_Dataset
...
def get_args():
    ...
    parser.add_argument("--save_interval", type=int, default=5, help="Number of epoches between testing phases")
    ...
    parser.add_argument("--train_set", type=str, default="seen",
                        help="For both VOC2007 and 2012,Training the model using seen classes only. You can change the split ratio of seen and unseen")
    parser.add_argument("--test_set", type=str, default="test_seen",
                        help="For both VOC2007 and 2012, seen in val/test")
    ...
    parser.add_argument("--pre_trained_model_path", type=str, default="trained_models/only_params_trained_yolo_voc_100")
    parser.add_argument("--log_path", type=str, default="tensorboard/SAN@100")
    parser.add_argument("--saved_path", type=str, default="trained_models/SAN@100")
    parser.add_argument("--split", type=str, default='1505', help='Ratio of the seen and unseen classes')
...
def train(opt):
    ...
    training_set = VOCZSD_Dataset(opt.data_path, opt.year, opt.split, opt.train_set, opt.image_size)
    training_generator = DataLoader(training_set, **training_params)

    test_set = VOCZSD_Dataset(opt.data_path, opt.year, opt.split, opt.test_set, opt.image_size, is_training=False)
    ...
    if torch.cuda.is_available():
        if opt.pre_trained_model_type == "model":
            ...
            #print('\n\n %%%%%%%% \n\n')
            model = Yolo(training_set.num_classes, is_san=is_san)
            model.load_state_dict(torch.load(opt.pre_trained_model_path),strict=False)
    ...
    if(not is_san):
        ...
        #intialize weight for the 2nd last layer of model
        nn.init.normal_(list(model.modules())[-2].weight,0,0.01)
    ...
    if(is_san):
        trainable_weights = [   'san.transform.weight'
                                , 'san.transform.bias'
                                , 'san.fc1.weight'
                                , 'san.fc1.bias']
                                #, 'san.fc2.weight']
        ...
        for i, j in model.named_parameters():
            if(i in trainable_weights):
                new_parameters+=[j]

        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, new_parameters), lr=1e-5, momentum=opt.momentum, weight_decay=opt.decay)
    else:
    ...
    for epoch in range(opt.num_epoches):
        ...
        for iter, batch in enumerate(training_generator):
            ...
            optimizer.zero_grad()
            # logits contains two scores : class conditional probablities and bounding box values with confidence score
            logits = model(image)
            loss, loss_coord, loss_conf, loss_cls = criterion(logits, label)
            
            loss.backward()
            optimizer.step()
            print("\nEpoch: {}/{}, Iteration: {}/{}, Lr: {}, Loss:{:.2f} (Coord:{:.2f} Conf:{:.2f} Cls:{:.2f})".format(
                epoch + 1,
                opt.num_epoches,
                iter + 1,
                num_iter_per_epoch,
        ...
        if epoch % opt.test_interval == 0:
            ...
            if epoch % opt.save_interval == 0:
                torch.save(model.state_dict(), opt.saved_path + os.sep + "only_params_trained_yolo_voc_" + str(epoch))
                #torch.save(model, opt.saved_path + os.sep + "whole_model_trained_yolo_voc_" + str(epoch))
